...
봉차트는 실제 `open`, `close` 값을 사용합니다. `selectedPrice`는 추세선 계산과 점선 라인 표시에만 사용됩니다.
...
| --- | --- | --- |
| `npm start` | `node index.js` | 샘플 주가 데이터를 생성하고 필터 결과와 `chart.html`을 생성합니다. |
| `npm run dev` | `node index.js` | 개발 중 같은 생성 과정을 실행합니다. 별도 dev server는 사용하지 않습니다. |
| `npm run generate` | `node index.js` | 정적 배포용 `chart.html`을 다시 생성합니다. |
...
| `index.js` | 실행 진입점, SVG 봉차트 HTML 생성 및 저장 |
...
차트 y축 스케일은 최근 N일의 `open`, `close`, `selectedPrice` 전체 min/max를 기준으로 잡습니다. 필터링 로직은 기존 요구대로 `selectedPrice` 기준 좌표 변환을 사용합니다.
